...
public class TopUpAdapter extends RecyclerView.Adapter<TopUpAdapter.TopupViewHolder> {
    ...
    public void onBindViewHolder(@NonNull TopupViewHolder holder, int position) {
        ...
        holder.txt_data.setText("₹ " +topUpResponse.getPrice()+" + 18% Taxes");
        holder.pay.setOnClickListener(view -> ((TopUpActivity)context).pay(topUpResponse));
        if(isConsumed){
        ...
        holder.txt_price.setText(topUpResponse.getData_volume());
    }
    @Override
